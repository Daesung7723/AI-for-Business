...
3. 반복 개선(후속 질문)으로 답변의 품질을 끌어올린다.
4. AI 사용 기본 수칙(초안 전제·출처 확인·AI 활용 표기·기밀·개인정보 입력 금지)을 자기 과제에 적용한다.
5. 조별 실습에서 같은 프롬프트에 대한 응답을 비교하고, 답이 달라진 지점과 확인되지 않은 지점을 기록한다.

1주에는 AI가 답을 만드는 원리를, 2주에는 AI가 아는 것과 모르는 것의 경계를 확인했습니다. 오늘은 관점을 AI에서 사용자로 옮겨, 같은 AI에게서 더 정확하고 유용한 답을 얻도록 **질문을 설계하는 방법**을 다룹니다. 다음 주 과제 1이 시작되므로, 오늘 배우는 틀과 수칙이 과제 수행의 기초가 됩니다.
...
AI가 스스로 추론하고 여러 에이전트가 작업을 나누어 처리하는 2026년에도 프롬프트 설계를 배워야 할까요? 본론에 앞서 AI 활용 방식이 어디까지 발전했는지 확인하고, 그 흐름에서 프롬프트가 차지하는 위치를 정리합니다.
...
- **GPT-3 연구의 발견** — 모델을 다시 학습하지 않고 과업 설명과 예시 몇 개를 **글로 입력하는 것만으로** 새 과업을 수행할 수 있음이 확인되었습니다.
- **변화의 의미** — 추가 학습 없이도 입력 문장만으로 모델에 새 과업을 지정할 수 있게 되었습니다. 어떤 예시를 어떤 형식으로 제시할지가 사용자가 설계할 대상이 된 것입니다.
...
- Wei 외는 중간 단계를 서술하게 하면 여러 단계의 문제를 단계별로 나누어 처리할 수 있고, 어려운 문제일수록 답에 이르기까지 더 많은 글을 생성하게 된다고 설명합니다.
- 같은 연구의 비교 실험에서는 **답을 먼저 낸 뒤** 풀이를 작성하게 하거나, 풀이 대신 식의 글자 수만큼 점(…)만 출력하게 하면 기본 방식과 비슷한 성능에 그쳤습니다.
...
- 2022년에 사용자가 프롬프트로 요구하던 중간 단계 서술이 **모델의 학습에 포함**된 것입니다. 같은 요구를 덧붙여 얻는 추가 효과가 줄어든 이유가 여기에 있습니다.
...
| 근거 | 확인된 내용 |
|------|------|
| OpenAI 문서 | 추론 모델에 "단계별로 생각하라"·"추론을 설명하라"는 요구는 불필요 · 예시 없이 먼저 작성 권고 |
| Google 문서 | Gemini 3 = 추론 모델 · 구형 모델용 복잡한 프롬프트 기법은 과도한 분석 유발 가능 |
| 측정 연구(2025-06) | 추론 모델 = 정확도 향상이 미미하거나 없음 · 응답 시간·토큰 크게 증가 |

- 같은 연구에서 추론 기능이 없는 모델은 평균 성능이 소폭 향상되었지만, 원래 맞히던 문항에서 오류가 생기는 등 답의 변동도 증가할 수 있었습니다. 단계 서술 요구의 효과는 **모델의 종류에 따라** 다릅니다.
...
- OpenAI의 한 개발팀은 에이전트 중심으로 작업하면서 팀의 주된 업무가 **환경 설계·의도 명시·피드백 구조 구축**으로 바뀌었다고 보고했습니다. 이때도 사람은 거의 전적으로 프롬프트를 통해 시스템과 상호작용했습니다.
...
기본적인 프롬프트 사용은 **요청 하나에 답 하나**를 받는 방식입니다. 에이전트(agent) 방식에서는 AI가 도구를 선택해 여러 단계를 스스로 수행합니다(2주 3.1). 2026년에는 작업의 단위가 한 번 더 확장되고 있습니다.
...
오케스트레이션(orchestration)은 **여러 에이전트에 작업을 분배하고 그 작업을 조율해 결과를 취합하는 일**입니다. IBM은 이를 디지털 교향악에 비유합니다 — 에이전트마다 고유한 역할이 있고, 오케스트레이터가 에이전트 간의 상호작용을 관리·조율합니다.

*(출처: 단계 구분 = Microsoft Azure Architecture Center 「AI Agent Orchestration Patterns」 · 정의 = IBM 「What is AI Agent Orchestration?」)*

**주요 개발사는 2026년에 오케스트레이션을 제품 기능으로 공개했습니다.**
...
세 제품의 공통 구조는 **분배 → 병렬 처리 → 취합**입니다. 각 에이전트는 담당한 작업만 처리하고, 결과를 통합하는 에이전트가 따로 있습니다.

*(출처: Anthropic 「New in Claude Managed Agents」 2026-05 · Google I/O 2026 발표 2026-05-20 · OpenAI 「에이전트 API 소개」 2026-09-10)*

**2026년 9월 현재 ① — 제품과 표준**

- 에이전트와 도구를 연결하는 표준 **MCP**(Model Context Protocol)와 에이전트와 에이전트를 연결하는 표준 **A2A**(Agent2Agent)가 Linux Foundation 산하 Agentic AI Foundation에 함께 소속되었습니다.
...
- Gartner는 완전 자율 에이전트가 **대부분의 기업 업무에 아직 준비되지 않았다**고 평가하고, 실질적인 에이전트 기능 없이 기존 제품을 에이전트로 재포장하는 **에이전트 워싱**(agent washing)을 경고합니다.
...
- **이유 ① 컨텍스트 창의 한계** — 한 대화에 담을 수 있는 분량에는 상한이 있어, 자료가 많은 작업일수록 한 창에서 처리할 때의 품질이 떨어집니다.
...
| 2025-04 보도 | 검색은 20~30건 수준에 머묾 · Microsoft 조사의 추가 고려 직무 중 최하위권 · 직함이 아닌 역량이라는 기업 임원(Nationwide CTO)의 평가 |
...
- 같은 보고서에 따르면 사람이 작업을 지시하고 판단하며 결과를 책임질 여지는 커지고 있습니다. 보고서는 **의도 설정**을 원하는 결과와 품질 기준을 정의하는 일로 설명합니다.
...
> **연결 포인트**
>
> 지난주에 도구마다 답이 다른 이유(학습 데이터·웹 검색)를 확인했습니다. 오늘의 질문은 이것입니다 — **같은 도구 안에서도 질문에 따라 답이 달라진다면, 좋은 질문이란 무엇인가?** 이 질문의 답을 조별 실습에서 직접 확인합니다.
