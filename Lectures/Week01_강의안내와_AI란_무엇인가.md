...
**A**: 유튜브·인스타그램의 추천, 지도 앱의 도착 예측, 사진 앱의 얼굴 분류 — 대부분의 학생이 이미 여러 번 사용해 온 AI입니다. 다만 이들은 오늘 배울 생성형 AI와는 **종류가 다른 AI**입니다. 무엇이 다른지는 3절과 6절에서 구분합니다.
...
단계가 올라갈수록 사람이 규칙을 작성하는 일은 줄고, **데이터가 결과를 결정하는 비중**이 커집니다. 오늘 우리가 다룰 대상이 이 마지막 ③단계 — 생성형 AI입니다.
...
③단계 생성형 AI는 앞선 ②단계의 기존 AI(추천·예측)와 결정적으로 다릅니다. 기존 AI가 정해진 답을 '고르고 판정한다'면, 생성형 AI는 문장·이미지를 '새로 만들어 낸다'는 점입니다. 두 종류의 자세한 비교는 6절에서 다루고, 여기서는 먼저 생성형 AI가 답을 만드는 원리부터 살펴봅니다.
...
답변은 **거절하지 않았고, 없는 내용을 새로 생성한 것도 아니었습니다.** 대신 질문의 전제를 바꾸어 답했습니다.
...
| 구분 | 예측형 AI (Predictive) | 생성형 AI (Generative) |
|---|---|---|
| 하는 일 | 과거 데이터의 패턴으로 **판별·점수·예측** | 학습한 패턴으로 **새로운 콘텐츠 생성** |
| 입력 → 출력 | 데이터 → **수치·등급·판정** (정해진 형식) | 질문·지시 → **문장·표·이미지** (열린 형식) |
...
출력이 **그 자리에서 새로 생성되는 문장**입니다. 형식이 열려 있고, 같은 질문에도 실행마다 표현이 조금씩 달라집니다(4.6 시연 ③의 원리). 예측형의 점수를 재료로 삼아 사람이 읽을 문장으로 바꿔 주는 관계이기도 합니다.
